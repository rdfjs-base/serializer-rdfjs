...
class PlainSerializer {
  ...
  transform (quads) {
    ...
    const { quadsString, stats } = [...quads].reduce((combined, quad) => {
      const termTypes = [quad.subject.termType, quad.predicate.termType, quad.object.termType]

      return {
        quadsString: combined.quadsString + this.serializeQuad(quad, blankNodes, namespaces) + ',\n',
        stats: {
          blankNodes: combined.stats.blankNodes || termTypes.includes('BlankNode'),
          literals: combined.stats.literals || termTypes.includes('Literal'),
          namedNodes: combined.stats.namedNodes || termTypes.includes('NamedNode'),
          total: combined.stats.total + 1
        }
      }
    }, { quadsString: '', stats: { blankNodes: false, literals: false, namedNodes: false, total: 0 } })
    ...
    return [
      ...
      '',
      ...this.writer.leadingLines(stats),
